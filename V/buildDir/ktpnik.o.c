...
typedef struct ktpnik__KtpNik ktpnik__KtpNik;
...
#pragma pack(push, 1)
typedef enum {
	ktpnik__KtpNikError__success = 0x00000000, // 0x00000000
	ktpnik__KtpNikError__insufficient_data = 0x86200001, // 0x86200001
	ktpnik__KtpNikError__invalid_date = 0x86200002, // 0x86200002
} __attribute__((packed)) ktpnik__KtpNikError;
#pragma pack(pop)
...
VV_LOCAL_SYMBOL  int compare_3338057098957291737_RepIndex_by_idx(RepIndex* a, RepIndex* b) {
...
ktpnik__KtpNikError ktpnik__ktpnik_parse(u64 nik, ktpnik__KtpNik* retval);
...
IError _const_none__; // inited later
 ...
 const u64 _const_ktpnik__min_nik_info = 1101010101000001U; // precomputed2
...
static int v_typeof_interface_idx_IError(int sidx) { /* IError */ 
	if (sidx == _IError_None___index) return 94;
	if (sidx == _IError_voidptr_index) return 2;
	if (sidx == _IError_Error_index) return 95;
	if (sidx == _IError_MessageError_index) return 97;
	return 30;
}
...
int strings__levenshtein_distance(string a, string b) {
	...
	for (int _t1 = 0; _t1 < a.len; ++_t1) {
		...
		for (int _t2 = 0; _t2 < b.len; ++_t2) {
			u8 cb = b.str[_t2];
			int _t3; /* if prepend */
			if ((int)((*(int*)array_get(f, j)) + 1) <= (int)((*(int*)array_get(f, (int)(j - 1))) + 1)) {
				_t3 = (int)((*(int*)array_get(f, j)) + 1);
			} else {
				_t3 = (int)((*(int*)array_get(f, (int)(j - 1))) + 1);
			}
			int mn =  _t3;
			if (cb != ca) {
				int _t4; /* if prepend */
...
void array_delete_many(array* a, int i, int size) {
	#if !defined(CUSTOM_DEFINE_no_bounds_checking)
	{
		if (i < 0 || (int)(i + size) > a->len) {
			...
			if (size > 1) {
				_t2 =  str_intp(2, _MOV((StrIntpData[]){{_SLIT(".."), /*100 &int*/0xfe07, {.d_i32 = (int)(i + size)}}, {_SLIT0, 0, { .d_c = 0 }}}));
			} else {
				_t2 = _SLIT("");
			}
			string endidx =  _t2;
...
VNORETURN VV_LOCAL_SYMBOL void panic_debug(int line_no, string file, string mod, string fn_name, string s) {
	...
	#else
	{
		...
		eprintln( str_intp(2, _MOV((StrIntpData[]){{_SLIT("   v hash: "), /*115 &string*/0xfe10, {.d_s = _SLIT("d474f69")}}, {_SLIT0, 0, { .d_c = 0 }}})));
		eprintln(_SLIT("========================================="));
		#if defined(_VNATIVE)
		{
			exit(1);
...
string winapi_lasterr_str(void) {
	...
	string _t2; /* if prepend */
	if (res == 0U) {
		_t2 =  str_intp(2, _MOV((StrIntpData[]){{_SLIT("Win-API error "), /*117 &u32*/0xfe06, {.d_u32 = err_msg_id}}, {_SLIT0, 0, { .d_c = 0 }}}));
	} else {
		_t2 = string_from_wide(msgbuf);
	}
	string err_msg =  _t2;
	return err_msg;
}
...
inline IError _v_error(string message) {
	;
	return /*&IError*/I_MessageError_to_Interface_IError(((MessageError*)memdup(&(MessageError){.msg = /*clone1*/ string_clone_static(message),.code = 0,}, sizeof(MessageError))));
}

// Attr: [inline]
inline IError error_with_code(string message, int code) {
	;
	return /*&IError*/I_MessageError_to_Interface_IError(((MessageError*)memdup(&(MessageError){.msg = /*clone1*/ string_clone_static(message),.code = code,}, sizeof(MessageError))));
}
...
string string_replace_each(string s, Array_string vals) {
	...
	qsort(idxs.data, idxs.len, idxs.element_size, (int (*)(const void *, const void *))&compare_3338057098957291737_RepIndex_by_idx);
	u8* b = malloc_noscan((int)(new_len + 1));
	int idx_pos = 0;
...
ktpnik__KtpNikError ktpnik__ktpnik_parse(u64 nik, ktpnik__KtpNik* retval) {
	if (nik < _const_ktpnik__min_nik_info) {
		ktpnik__KtpNikError _t1 = ktpnik__KtpNikError__insufficient_data;
		return _t1;
	}
	retval->province = ((u8)((u64)(((u64)(nik / 100000000000000U)) % 100U)));
	retval->city = ((u8)((u64)(((u64)(nik / 1000000000000U)) % 100U)));
	retval->region = ((u8)((u64)(((u64)(nik / 10000000000U)) % 100U)));
	retval->birthdate = ((u8)((u64)(((u64)(nik / 100000000U)) % 100U)));
	retval->birthmonth = ((u8)((u64)(((u64)(nik / 1000000U)) % 100U)));
	retval->birthyear = ((u8)((u64)(((u64)(nik / 10000U)) % 100U)));
	retval->uid = ((u16)((u64)(((u64)(nik / 1U)) % 10000U)));
	retval->is_female = retval->birthdate > 40;
	if (retval->is_female) {
		retval->birthdate -= 40;
	}
	if (retval->birthdate > 31 || retval->birthmonth > 12) {
		ktpnik__KtpNikError _t2 = ktpnik__KtpNikError__invalid_date;
		return _t2;
	}
	ktpnik__KtpNikError _t3 = ktpnik__KtpNikError__success;
	return _t3;
}
